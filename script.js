...
// Common light patterns for popular devices
...
const CUSTOMER_INITIATED_CALL_TYPES = [
  "Modem Offline",
  "Slow Connection",
  "Intermittent Connection",
];
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const ticketExtractionSection = document.getElementById(
    "ticketExtractionSection"
  );
  ...
  function saveToLocalStorage() {
    ...
    const selectedActions = Array.from(
      document.querySelectorAll(
        '#actionsContainer input[type="checkbox"]:checked'
      )
  ...
  function loadFromLocalStorage() {
    ...
    const modemLightData = JSON.parse(
      localStorage.getItem("modemLightData") || "{}"
    );
    ...
    const selectedActions = JSON.parse(
  ...
  function addClearFormButton() {
    ...
    clearButton.addEventListener("click", function () {
      if (
        ...
        updateUIByCallType(
          (document.getElementById("callType").value = "Courtesy Check")
        );
  ...
  function initializeUI() {
    ...
    for (const [lightNum, states] of Object.entries(MODEM_LIGHTS)) {
      ...
      lightContainer.setAttribute("data-light", lightNum);
    ...
    // Setup optional lights checkboxes
    const enableLightCheckboxes = document.querySelectorAll(".enable-light");
    enableLightCheckboxes.forEach((checkbox) => {
      checkbox.addEventListener("change", (e) => {
        const lightNum = e.target.id.replace("enableLight", "");
        const lightElement = document.querySelector(
          `.light-status-item[data-light="${lightNum}"]`
        );
        if (lightElement) {
          lightElement.style.display = e.target.checked ? "block" : "none";
        }
      });
    });
  }
  ...
  function extractInfo() {
    ...
    let trxMatch = inputText.match(
      /(?:TrxID|TID|Transaction ID)\s*:?\s*(\d{9})/i
    );
    ...
    document.getElementById("customerName").value = nameMatch
      ? nameMatch[1].trim()
      : "";
    document.getElementById("lsAccount").value = accountMatch
      ? accountMatch[1]
      : "";
    document.getElementById("ticketNum").value = ticketMatch
      ? ticketMatch[1]
      : "";
    document.getElementById("transactionId").value = trxMatch
      ? trxMatch[1]
      : "";
    document.getElementById("rogersId").value = rogersMatch
      ? rogersMatch[1]
      : "";
    document.getElementById("customerNum").value = customerNumMatch
      ? customerNumMatch[1]
      : "";
  ...
  function generateNote() {
    ...
    const customerActions = Array.from(
      actionsContainer.querySelectorAll('input[type="checkbox"]:checked')
    )
      .map((checkbox) => checkbox.value)
      .join("\n");
    ...
    if (MODEM_STATUS_CALL_TYPES.includes(callType)) {
      modemStatus = Object.keys(MODEM_LIGHTS)
        ...
        .map(
          (lightNum) =>
            `${lightNum}: ${document.getElementById(`modem-${lightNum}`).value}`
        )
        .join("\n");
    }
    ...
    let note = CALL_TYPES[callType]
      ...
      .replace(
        "{speed_test}",
        document.getElementById("speedTest").value || ""
      );
  ...
  function createLightStatusItem(lightNum) {
    const item = document.createElement("div");
    item.className = "light-status-item";
    item.setAttribute("data-light", lightNum);

    const label = document.createElement("label");
    label.textContent = `Light ${lightNum}:`;

    const select = document.createElement("select");
    select.className = "light-status-select";

    MODEM_LIGHTS[lightNum].forEach((state) => {
      const option = document.createElement("option");
      option.value = state;
    ...
    indicator.classList.add(LIGHT_STATE_CLASSES[select.value]);

    select.addEventListener("change", () => {
      updateLightIndicator(lightNum, select.value);
    });
